...
class Cube:
    ...
    def rotate_face(self, f, n=1, layer=0, rotation_matrix = 1):
        """Rotate Face"""
        ...
        axis = np.abs(v).argmax()
        sign = np.sign(v[axis])
        # print(f,axis,sign)
        f = self.revfacesdict[sign*(axis+1)]
        # print(f)
...
class InteractiveCube(plt.Axes):
    def __init__(self, cube=None,
        ...
        # Define movement for w/s or up/down mouse movement
        self._ax_pitch = (1, 0, 0)
        self._step_pitch = 0.03

        # Define movement for a/d or left/right mouse movement
        self._ax_yaw = (0, -1, 0)
        self._step_yaw = 0.03
        
        # Define movement for q/e movement
        self._ax_roll = (0,0, -1)
        self._step_roll = 0.03
        ...
        self._control = False  # shift key pressed
    ...
    def _initialize_widgets(self):
        ...
        self._btn_slots = []
        i = 0
        slot_funcs = [self._record_slot_0]
        self.now_recording = -1
        self.recordings = [None for x in range(slot_num)]
        self.is_playing = 0
        
        self._ax_slots.append(self.figure.add_axes([0.75, 0.05 + 0.1*(slot_num-i), 0.2, 0.075]))
        self._btn_slots.append(widgets.Button(self._ax_slots[-1], 'slot ' + str(i)))

        # self._btn_slots[-1].on_clicked(set_slot_id[i])
        self._btn_slots[-1].on_clicked(slot_funcs[0])

    def _record_slot_0(self, *args):
        # print('slot0')
        # print(args[0].key)
        self._record_slot(args,slot_id = 0)
    
    def _record_slot(self, args, slot_id):
        if self.is_playing:
            return
        if self.now_recording < 0: #start/play/delete recording slot_id
            if args[0].key == 'shift':
                # print('delete recording')
                self.recordings[slot_id] = None
            elif self.recordings[slot_id]:
                # print('playing recording', slot_id)
                # print(self.recordings[slot_id][1])
                M = self.recordings[slot_id][0].as_rotation_matrix().transpose()
                # print(M)
                # print(self._current_rot.as_rotation_matrix())
                M = M.dot(self._current_rot.as_rotation_matrix())
                # print(M)
                self.is_playing = 1
                for move in self.recordings[slot_id][1]: #iterate move list
                    self.rotate_face(move[0],move[1],move[2],steps=3,rotation_matrix = M)
                self.is_playing = 0
            else:    
                # print('start recording', slot_id)
                self.now_recording = slot_id
                # save current orientation and initialize a move list.
                self.recordings[self.now_recording] = [self._current_rot,[]]
                
        elif self.now_recording == slot_id: #stop recording slot_id
            # print('stop recording', slot_id)
            self.now_recording = -1
    ...
    def rotate_face(self, face, turns=1, layer=0, steps=5, rotation_matrix = 1):
        import time
        if not np.allclose(turns, 0):
            for i in range(steps):
                self.cube.rotate_face(face, turns * 1. / steps,
                                      layer=layer, rotation_matrix = rotation_matrix)
                time.sleep(0.02)
                self._draw_cube()
        if self.now_recording > -1:
            self.recordings[self.now_recording][1].append((face,turns,layer))
            print('recorded ',(face,turns,layer), ' to ', self.now_recording)
        else:
    ...
    def _key_press(self, event):
        ...
        if event.key == 'control':
            ...
            self.rotate(Quaternion.from_v_theta(self._ax_yaw,
                                                5 * self._step_yaw))
        elif event.key == 'a':
            self.rotate(Quaternion.from_v_theta(self._ax_yaw,
                                                -5 * self._step_yaw))
        elif event.key == 'w':
            self.rotate(Quaternion.from_v_theta(self._ax_pitch,
                                                5 * self._step_pitch))
        elif event.key == 's':
            self.rotate(Quaternion.from_v_theta(self._ax_pitch,
                                                -5 * self._step_pitch))
        elif event.key == 'q':
            self.rotate(Quaternion.from_v_theta(self._ax_roll,
                                                5 * self._step_roll))
        elif event.key == 'e':
            self.rotate(Quaternion.from_v_theta(self._ax_roll,
                                                -5 * self._step_roll))
    ...
    def _mouse_press(self, event):
        ...
        # print(self._face_polys[1].contains(event))
        minzorder = -np.inf
        self._face_event = -1
        for i in range(len(self._face_polys)):
            if self._face_polys[i].contains(event)[0] and \
                self._face_polys[i].zorder > minzorder:
                self._face_event = i
                minzorder = self._face_polys[i].zorder
        self._event_xy = (event.x, event.y)
        if event.button == 1:
            self._button1 = True
        elif event.button == 3:
    ...
    def _mouse_motion(self, event):
        ...
        if self._button1 or self._button2:
            # face_clicked = self._in_face(self._event_xy)
            dx = event.x - self._event_xy[0]
            dy = event.y - self._event_xy[1]
            if self._face_event >= 0:
                # face_event = self._face_event
                
                print('rotate', self._face_event)
                vertices = self._face_polys[self._face_event].get_xy()
                edges = [(vertices[i+1][0]-vertices[i][0],vertices[i+1][1]-vertices[i][1]) for i in range(len(vertices)-1)]
                edges = [x/np.sqrt(np.dot(x,x)) for x in edges]
                edges_score = np.array([dx*edge[0]+dy*edge[1] for edge in edges])
                
                edge_d = edges[edges_score.argmax()]
                # print(dx,dy)
                # print(edge_d)
                
                vertices = self.cube._faces[self._face_event]
                edges = [(vertices[i+1][0]-vertices[i][0],
                          vertices[i+1][1]-vertices[i][1],
                          vertices[i+1][2]-vertices[i][2]) for i in range(len(vertices)-1)]
                ...
                # print(edge_d)
                face = ''
                centroid_d= np.array(self.cube._face_centroids[self._face_event, :3])
                face_d = np.array(centroid_d)
                face_d[(face_d>-0.9)* (face_d<0.9)]=0
                
                v = -np.array([face_d[1]*edge_d[2]-face_d[2]*edge_d[1],
                               face_d[2]*edge_d[0]-face_d[0]*edge_d[2],
                               face_d[0]*edge_d[1]-face_d[1]*edge_d[0]])
                ...
                if v[0]>0.5:
                    face = 'R'
                    if centroid_d[0] > epsilon:
                        layer = 0
                    elif centroid_d[0] < -epsilon:
                        layer = 2
                    else:
                        layer = 1
                if v[0]<-0.5:
                ...
                if v[1]>0.5:
                    face = 'U'
                    if centroid_d[1] > epsilon:
                        layer = 0
                    elif centroid_d[1] < -epsilon:
                        layer = 2
                    else:
                        layer = 1
                if v[1]<-0.5:
                ...
                if v[2]>0.5:
                    face = 'F'
                    if centroid_d[2] > epsilon:
                        layer = 0
                    elif centroid_d[2] < -epsilon:
                        layer = 2
                    else:
                        layer = 1
                if v[2]<-0.5:
                ...
                self._face_event = -2
                self.rotate_face(face,1,layer)
                
                
            #     move_cube
            elif self._face_event >= -1:
                
                self._event_xy = (event.x, event.y)    
                rot1 = Quaternion.from_v_theta(self._ax_pitch,
                                               self._step_pitch * dy)
                rot2 = Quaternion.from_v_theta(self._ax_yaw,
                                               self._step_yaw * dx)
